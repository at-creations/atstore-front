...
export function ProductList() {
  ...
  useEffect(() => {
    async function loadProducts() {
      ...
      try {
        ...
        const fetchedProductsResponse = selectedCategory === "All"
          ? await fetchProductsByCategory("", itemsPerPage, offset)
          : await fetchProductsByCategory(selectedCategory, itemsPerPage, offset)
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="text-2xl font-semibold mb-4 text-gray-700 dark:text-gray-200">Categories</h2>
        <div className="flex flex-wrap gap-4">
          {isLoadingCategories
            ? Array.from({ length: 5 }).map((_, index) => <SkeletonButton key={index} />)
            : <>
                <Button
                ...
                {categories.map((category) => (
                  <Button
                    key={category._id}
                    variant={selectedCategory === category._id ? "primary" : "secondary"}
